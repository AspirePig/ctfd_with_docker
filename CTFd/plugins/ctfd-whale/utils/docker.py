...
class DockerUtils:
    ...
    def _create_standalone_container(client, container):
        ...
        client.containers.run(image=container.challenge.docker_image, name=f'{container.user_id}-{container.uuid}',
                              environment={'FLAG': container.flag}, detach=True,
                              network=get_config("whale:docker_auto_connect_network", "ctfd_frp-containers"),
                              mem_limit=DockerUtils.convert_readable_text(container.challenge.memory_limit),
                              nano_cpus=int(container.challenge.cpu_limit * 1e9), auto_remove=True,
                              pids_limit=200)
...
#        )
    ...
    def remove_container(container):
        ...
        networks = client.networks.list(names=[str(container.user_id) + '-' + container.uuid])

        if len(networks) == 0:
            containers = client.containers.list(filters={'name': str(container.user_id) + '-' + container.uuid})
            for c in containers:
                c.remove(v=True, force=True)
        else:
            containers = client.containers.list(filters={'label': str(container.user_id) + '-' + container.uuid})
            for c in containers:
                c.remove(v=True, force=True)
            for n in networks:
