...
class UserContainers(Resource):
    ...
    def get():
        ...
        targetdc = client.containers.get(str(user_id) + "-" + container.uuid)
        ip_add = targetdc.attrs['NetworkSettings']['Networks'][get_config("whale:docker_auto_connect_network", "ctfd_frp-containers")]['IPAddress']
        return {
            'success': True,
            'data': {
                'lan_domain': ip_add,
